...
@section('title')
Nuevo Utensilio Para {{ ucwords($receta->nombre) }}
@stop
...
<div class="row justify-content-center">       
    <div class="card" style="width: 50em;">
        <card-header><h1>Nuevo Utensilio Para {{ ucwords($receta->nombre) }}</h1></card-header>
        <div class="card-body">
            <form method="POST" action="{{ route('admin.receta_utensilios.store') }}" autocomplete="false">
                <input name="_token" id="_token" value="{{ csrf_token() }}" type="hidden">                
                <div class="form-group">
                    <label for="utensilio_id" class="col col-form-label text-md-left">Ingrediente<span style="color:red">*</span></label>
                    <div class="col">                        
                        <select class="input-select2 form-control" name="utensilio_id" id="utensilio_id" style="width: 100%;">
                            ...
                            @foreach ($utensilios as $utensilio)
                                <option value="{{$utensilio->id}}" {{ (old('utensilio_id')==$utensilio->id)?"selected":"" }}>{{$utensilio->nombre}}</option>
                            @endforeach
                        </select>
                        @error('utensilio_id')
